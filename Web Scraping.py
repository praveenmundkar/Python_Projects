...
for cat in category:
    ...
    print(list_of_sub_jobs)                                                  # Jobs under the category printed before
    nn += 1
    print("-"*50)
    ...
    for link in links:
        ...
        try:
            ...
            fe = driver.find_elements(By.XPATH, "//dd[contains(@class,'basic-info-item__description')]")                                         # Scraping ther info. i.e no. of employees and headquater location
            for fe in fe:
                # print(fe.text)
                f_e.append(fe)
            ...
            driver.close()                                                                      # Closing the child window which is now no. longer required to avoid some errors
            driver.switch_to.window(parentwindow)

        except:
            print("occured some error")
            pass
